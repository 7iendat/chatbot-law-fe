...
// // src/hooks/useRouteGuard.ts (thay thế toàn bộ file cũ)
...
//     redirectTo?: string;
//     requireAuth?: boolean;
//     adminOnly?: boolean; // Thêm lại option này
// }
...
//     const {
//         redirectTo = "/login", // Mặc định chuyển về trang login
//         requireAuth = true,
//         adminOnly = false,
//     } = options;
...
//             return; // Chờ cho đến khi AuthContext xác định xong người dùng là ai.
//         }
...
//             // Chuyển hướng nếu user đã đăng nhập mà lại vào trang public
//             if (publicPaths.includes(pathname)) {
//                 console.log(
//                     `RouteGuard: Authenticated user on public page. Redirecting to dashboard...`
//                 );
//                 router.replace("/"); // hoặc trang chính
//                 return;
//             }
...
//                 );
//                 router.replace("/unauthorized"); // Trang báo lỗi không có quyền
//                 return;
//             }
//         }
//     }, [
...
//     // Trả về trạng thái để component có thể quyết định hiển thị gì
//     const isLoading = !isInitialized || authLoading;
//     const isAuthorized = !requireAuth || isAuthenticated; // Logic đơn giản: được quyền nếu không yêu cầu auth hoặc đã auth
...
// // Các helper hook giữ nguyên hoặc điều chỉnh cho phù hợp
// export const useAdminGuard = (redirectTo?: string) => {
//     return useRouteGuard({
//         requireAuth: true,
//         adminOnly: true,
...
//     // Tính toán trạng thái ủy quyền
//     let isAuthorized = false;
//     if (!isLoading) {
//         if (!requireAuth) {
//             isAuthorized = true;
//         } else if (isAuthenticated) {
//             // Đã đăng nhập, kiểm tra vai trò nếu cần
//             const currentUserRole = user?.role?.toLowerCase();
//             if (
//                 requiredRole &&
//                 currentUserRole !== requiredRole.toLowerCase()
//             ) {
//                 isAuthorized = false; // Sai vai trò
//             } else {
//                 isAuthorized = true; // Đúng vai trò hoặc không yêu cầu vai trò cụ thể
//             }
//         } else {
...
//         isLoading,
//         isAuthorized,
//         isAuthenticated, // Vẫn trả về để component có thể dùng
//         user,
//     };
// };
